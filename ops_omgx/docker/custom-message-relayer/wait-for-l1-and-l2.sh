...
export L1_WALLET_KEY=`/opt/secret2env -name $SECRETNAME|grep -w CUSTOM_RELAY_L1_WALLET_PRIV_KEY|sed 's/CUSTOM_RELAY_L1_WALLET_PRIV_KEY=//g'`
...
if [ ! -z "$DEPLOYER_HTTP" ]; then
    ...
    exec env \
        ...
        L1_WALLET_KEY=`/opt/secret2env -name $SECRETNAME|grep -w CUSTOM_RELAY_L1_WALLET_PRIV_KEY|sed 's/CUSTOM_RELAY_L1_WALLET_PRIV_KEY=//g'` \
