...
const logBalance = (provider: WebSocketProvider, blockNumber: number, networkName: configs.OMGXNetwork) => {
  const promiseData = networkName === configs.OMGXNetwork.L1 ? [
...
const logTransaction = (socket: WebSocketProvider, trans: TransactionResponse, networkName: configs.OMGXNetwork, metadata = {}) => {
  // check from/to address is pool address
  const poolAddress = (networkName === configs.OMGXNetwork.L1) ? configs.l1PoolAddress : configs.l2PoolAddress
  logger.debug({
    from: trans.from,
    to: trans.to,
    poolAddress
...
const logData = (socket: WebSocketProvider, blockNumber: string, networkName: configs.OMGXNetwork) => {
  const poolAddress = (networkName === configs.OMGXNetwork.L1) ? configs.l1PoolAddress : configs.l2PoolAddress
  const metadata = {
    blockNumber,
    networkName,
...
const onConnected = (networkName: configs.OMGXNetwork) => {
...
const onError = (networkName: configs.OMGXNetwork, provider: WebSocketProvider) => {
