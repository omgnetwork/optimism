...
export const run = async () => {
  ...
  const environment = config.str('node-env', env.NODE_ENV) || 'production'
  const network = config.str('eth-network-name', env.ETH_NETWORK_NAME) || 'rinkeby'
  const release = `batch-submitter@1.0.0` 
