...
const downloadBidOffer = (bidDecryptionWaitingList) => (dispatch) => {
  ...
  if (bidDecryptionWaitingList[0].bidOffer.length !== 0) {
    let bidOffers = orderBy(
      bidDecryptionWaitingList[0].bidOffer,
      'timestamp',
      'desc'
    )
  ...
  workerInstance.addEventListener('message', (message) => {
    if (
      ...
      if (bidCleartext[511] === 0) {
        const buyerItemToReceiveAmount = accDiv(
          bidCleartext[8],
          Math.pow(10, 5)
        )
        ...
        AESEncrypt(JSON.stringify(cleartext), AESKey).then((ciphertext) => {
          ...
          if (bidOfferDataCache)
            bidOfferDataCache = JSON.parse(bidOfferDataCache)
          if (!bidOfferDataCache) {
          ...
          localStorage.setItem(
            'bidOfferData',
            JSON.stringify(bidOfferDataCache)
          )
        })
        ...
        oracleService.publish({
          ...
          amountB: accMul(buyerExchangeRate, buyerItemToReceiveAmount).toFixed(
            5
          ),
...
export const getBidOffer = (bidDecryptionWaitingList, FHEseed, AESKey) => (
  dispatch
) => {
    ...
    if (itemOpenOrClosed[eachAskOpen].bidOffer.length !== 0) {
      ...
      if (bidOfferDataCache) {
        ...
        if (bidOfferData.length !== 0) {
          bidDecryptionWaitingList.push({
            itemID: eachAskOpen,
            bidOffer: bidOfferData,
          })
        }
      } else {
        bidDecryptionWaitingList.push({
          itemID: eachAskOpen,
          bidOffer: itemOpenOrClosed[eachAskOpen].bidOffer,
        })
      }
    }
  }
...
export const decryptBidOfferCache = (
  itemOpenOrClosed,
  itemOpenList,
  bidOfferDataCache,
    ...
    if (itemOpenOrClosed[eachAskOpen].bidOffer.length !== 0) {
      for (let eachBidOffer of itemOpenOrClosed[eachAskOpen].bidOffer) {
        ...
        if (decryptBidOfferCacheError[eachAskOpen]) {
          if (
            typeof decryptBidOfferCacheError[eachAskOpen][
              eachBidOffer.bidID
            ] === 'undefined'
          ) {
        ...
        if (BidOfferInCache && !BidOfferDecrypted) {
          ...
          try {
            const base64Ciphertext =
              bidOfferDataCache[eachAskOpen][eachBidOffer.bidID].ciphertext
            const base64IV =
              bidOfferDataCache[eachAskOpen][eachBidOffer.bidID].iv
            const ciphertText = {
