...
import { addNFT, getNFTs } from 'actions/nftAction'
...
class NetworkService {
        ...
        const filteredTransactions = transactions.filter(
          (i) =>
            [
              ...
              this.L2DepositedERC20Address.toLowerCase(),
              this.l2ETHGatewayAddress.toLowerCase(),
  ...
  async approveErc20(
    value,
    currency,
    approveContractAddress = this.L1ERC20GatewayAddress,
  ...
  async depositErc20(value, currency, gasPrice) {
    try {
      ...
      const depositTxStatus = await this.OVM_L1ERC20Gateway.deposit(value, {
        gasLimit: 1000000,
      })
      await depositTxStatus.wait()
  ...
  async exitOMGX(currency, value) {
    if (currency === this.l2ETHGatewayAddress) {
      ...
      const [l2ToL1msgHash] = await this.watcher.getMessageHashesFromL2Tx(
        tx.hash
      )
  ...
  async withdrawLiquidity(currency, value, L1orL2Pool) {
    ...
    try {
      ...
      const withdrawLiquidityTX = await await (L1orL2Pool === 'L1LP'
        ? this.L1LPContract
        : this.L2LPContract
      ).withdrawLiquidity(withdrawAmount, currency, this.account)
