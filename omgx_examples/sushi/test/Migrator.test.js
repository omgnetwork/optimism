...
describe("Migrator", function () {
  ...
  beforeEach(async function () {
    this.factory1 = await this.Factory__UniswapV2Factory.deploy(bob.address, {gasLimit: 800000, gasPrice: 0})
    await this.factory1.deployTransaction.wait()

    this.factory2 = await this.Factory__UniswapV2Factory.deploy(bob.address, {gasLimit: 800000, gasPrice: 0})
    ...
    this.weth = await this.Factory__ERC20Mock.deploy("WETH", "WETH", "100000000", {gasLimit: 800000, gasPrice: 0})
    await this.weth.deployTransaction.wait()

    this.token = await this.Factory__ERC20Mock.deploy("TOKEN", "TOKEN", "100000000", {gasLimit: 800000, gasPrice: 0})
    await this.token.deployTransaction.wait()

    const pair1 = await this.factory1.createPair(this.weth.address, this.token.address, {gasLimit: 800000, gasPrice: 0})
    const pair1TX = await pair1.wait()

    this.lp1 = await this.Factory__UniswapV2Pair.attach(pair1TX.events[1].args.pair)

    const pair2 = await this.factory2.createPair(this.weth.address, this.token.address, {gasLimit: 800000, gasPrice: 0})
    ...
    const transfer = await this.sushi.transferOwnership(this.chef.address, {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()

    const add = await this.chef.add("100", this.lp1.address, true, {gasLimit: 800000, gasPrice: 0})
    await add.wait()
  })

  it("should do the migration successfully", async function () {
    let transfer
    transfer = await this.token.transfer(this.lp1.address, "10000000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    transfer = await this.weth.transfer(this.lp1.address, "500000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    const mint = await this.lp1.mint(minter.address, {gasLimit: 800000, gasPrice: 0})
    await mint.wait()
    expect(await this.lp1.balanceOf(minter.address)).to.equal("2235067")

    // Add some fake revenue
    transfer = await this.token.transfer(this.lp1.address, "100000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    transfer = await this.weth.transfer(this.lp1.address, "5000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    const sync = await this.lp1.sync({gasLimit: 800000, gasPrice: 0})
    await sync.wait()
    ...
    let setMigrate
    setMigrate = await this.chef.setMigrator(this.migrator.address, {gasLimit: 800000, gasPrice: 0})
    await setMigrate.wait()
    await expect (this.chef.migrate(0)).to.be.eventually.rejected;

    setMigrate = await this.factory2.setMigrator(this.migrator.address, {gasLimit: 800000, gasPrice: 0})
    await setMigrate.wait()
    migrate = await this.chef.migrate(0, {gasLimit: 800000, gasPrice: 0})
    await migrate.wait()
    ...
    const withdraw = await this.chef.connect(minter).withdraw("0", "2000000", {gasLimit: 800000, gasPrice: 0})
    await withdraw.wait()
    transfer = await this.lp2.connect(minter).transfer(this.lp2.address, "2000000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    const burn = await this.lp2.burn(bob.address, {gasLimit: 800000, gasPrice: 0})
    await burn.wait()
  ...
  it("should allow first minting from public only after migrator is gone", async function () {
    const setMigrate = await this.factory2.setMigrator(this.migrator.address, {gasLimit: 800000, gasPrice: 0})
    await setMigrate.wait()

    this.tokenx = await this.Factory__ERC20Mock.deploy("TOKENX", "TOKENX", "100000000", {gasLimit: 800000, gasPrice: 0})
    await this.tokenx.deployTransaction.wait()

    const pair = await this.factory2.createPair(this.weth.address, this.tokenx.address, {gasLimit: 800000, gasPrice: 0})
    const pairTX = await pair.wait()

    this.lpx = await this.Factory__UniswapV2Pair.attach(pairTX.events[1].args.pair)

    let transfer
    transfer = await this.weth.connect(minter).transfer(this.lpx.address, "10000000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
    transfer = await this.tokenx.connect(minter).transfer(this.lpx.address, "500000", {gasLimit: 800000, gasPrice: 0})
    await transfer.wait()
